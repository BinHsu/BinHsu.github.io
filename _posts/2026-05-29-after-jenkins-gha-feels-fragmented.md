...
**Why Port and not the hotter Backstage?** Backstage is the name everyone reaches for — the CNCF darling, the default answer. But here's the trap, and it's the whole point of this post: **Backstage is heavier to maintain than the Jenkins you're trying to leave.** Jenkins, for all its age, is a stable CI server — you set up pipelines and they run. Backstage is a bespoke React/Node application you own *as source*: a plugin tree on a fast-moving npm core, breaking changes between versions, upgrades teams dread and postpone. You're not adopting a product; you're signing up to build and operate a custom web app. So reaching for Backstage to escape platform-maintenance moves you in the wrong direction — toward *more* of it. Port earns the default precisely because it deletes that burden: managed core, a catalog that builds itself, no app to keep alive. Reach for Backstage only when you genuinely need to *own* the control plane and have a platform team to keep it breathing — not because it's the one on everyone's slides.
